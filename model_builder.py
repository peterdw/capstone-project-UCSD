...
import music_player
from constants import KEY_ORDER
...
def create_music_generation_lstm_model(hp):
    ...
    loss_weights = {
        'pitch': 1.0,  # prioritization of pitch accuracy
        'step': 0.5,
        'duration': 0.5
    }
...
def create_music_generation_transformer_model(hp):
    ...
    transformer_block = tfa.layers.TransformerEncoderLayer(
        d_model=d_model,
        num_heads=num_heads,
        dropout=0.1,
    )
...
def create_music_sequence_dataset(dataset: tf.data.Dataset, seq_length: int, vocab_size=128) -> tf.data.Dataset:
    ...
    # Define a function to normalize note pitch within sequences.
    def scale_pitch(x):
        x = x / [vocab_size, 1.0, 1.0]  # Normalize only the pitch dimension.
        return x

    # Define a function to split inputs from labels within sequences.
...
def create_midi_training_dataset(filenames: list[str], num_files: int = None) -> tf.data.Dataset:
    ...
    for f in filenames[:num_files]:
        notes = music_player.midi_to_notes(f)
